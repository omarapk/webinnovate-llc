...
<div class="row g-3">
    ...
    <div class="col-md-6">
        <label for="app_slug" class="form-label fw-semibold">App <span class="text-danger">*</span></label>
        <select name="app_slug" id="app_slug" class="form-select @error('app_slug') is-invalid @enderror" required>
            @foreach (\App\Support\AppCatalog::all() as $catalogApp)
                <option value="{{ $catalogApp['slug'] }}" @selected(old('app_slug', $post->app_slug ?? 'leadform') === $catalogApp['slug'])>{{ $catalogApp['name'] }}</option>
            @endforeach
        </select>
        <div class="form-text">Which app's blog this article belongs to (/{app}/blog).</div>
        @error('app_slug')
            <div class="invalid-feedback">{{ $message }}</div>
        @enderror
    </div>
